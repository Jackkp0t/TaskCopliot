...
export function App() {
  ...
  const createManual = async (value: Partial<ClientTask> & { title: string }) => { setBusy(true); try { await api.createTask(value); await loadTasks(); } catch (err) { setError(err instanceof Error ? err.message : '保存任务失败'); } finally { setBusy(false); } };
